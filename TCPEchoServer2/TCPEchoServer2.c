...
#define MAXPENDING 1	//number of Max pending of request
...
//bind to local address
	if((bind(servSock, (struct sockaddr *) &echoServAddr, sizeof(echoServAddr)))==-1) exit(1);
//listen
	if((listen(servSock,MAXPENDING))==-1) exit(1);

//infinite loop
	while(1){
		clntLen = sizeof(echoClntAddr);
        sleep(100);
		if((clntSock = accept(servSock, (struct sockaddr *) &echoClntAddr, &clntLen))==-1) exit(1);
